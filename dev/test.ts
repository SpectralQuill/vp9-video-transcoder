...
async function main() {
    const
        ...
        convertedVideoNames = convertedVideoPaths.map(fp => path.basename(fp)),
        videosToConvert: EpisodeRecords = {}
    ;
    for (const code in episodeData) {
        const
            rawIndex = rawVideoNames.findIndex(name => name.includes(code)),
            convertedIndex = convertedVideoNames.findIndex(name => name.includes(code)),
            hasRaw = (rawIndex !== -1),
